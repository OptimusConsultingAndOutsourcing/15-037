...
;(function() {
	...
	var instanceMethods = {
		...
		diffText: function(compare) {
			...
			if (diff < 120) {
				...
				return seconds >= 0 ? 'en un momento' : 'un momento atras';

			} else if (diff < 3600) {
				// 2 to 59 minutes ago
				rawText = Math.floor(diff / 60) + " minutos";

			} else if (diff < 86400) {
				// less than 24 hours ago
				var hours = Math.floor(diff / 3600);
				var s = hour == 1 ? '' : 's';
				rawText = hours + " hora" + s + " atras";

			} else if (diff < 172800) {
				// yesterday
				return seconds > 0 ? 'ma&ntilde;ana' : 'ayer';

			} else if (diff < 604800) {
				// 2 to 6 days ago
				rawText = Math.floor(diff / 86400) + " dias";

			} else if (diff < 1209600) {
				// within 14 days
				return seconds > 0 ? 'proxima semana' : 'semana anterior';

			} else if (diff < 2419200) {
				// within 28 days
				rawText = Math.floor(diff / 604800) + " semanas";

			} else if (diff < 5184000) {
				// within 60 days
				return seconds > 0 ? 'proximo mes' : 'mes anterior';

			}	else if (diff < 31536000) {
				// within 365 days
                var months=Math.floor(diff / 2592000);
                var s = months == 1 ? '' : 'es';
				rawText = months + " mes"+s;

			} else if (diff < 63072000) {
				// within 730 days
				return seconds > 0 ? 'proximo a&ntilde;o' : 'a&ntilde;o anterior';

			} else {
				// years ago
                var years=Math.floor(diff / 31536000);
                var s = years == 1 ? '' : 's';
				rawText = years + " a&ntilde;o"+s;
	...
	var staticMethods = {
		 ...
		 */			
		MONTHNAMES      : 'Enero Febrero Marzo Abril Mayo Junio Julio Agosto Septiembre Octubre Noviembre Deciembre'.split(' '),
		/**
		 * @var {Array}  Abbreviated names for the months of the year
		 */	
		ABBR_MONTHNAMES : 'Ene Feb Mar Abr May Jun Jul Ago Sep Oct Nov Dic'.split(' '),
		/**
		 * @var {Array}  Names for the days of the week from Sunday to Saturday
		 */			
		DAYNAMES        : 'Domingo Lunes Martes Miercoles Jueves Viernes S&aacute;bado'.split(' '),
		/**
		 * @var {Array}  Abbreviated names for the days of the week from Sunday to Saturday
		 */		
		ABBR_DAYNAMES   : 'Dom Lun Mar Mie Jue Vie Sab'.split(' '),
		/**
		 * @var {Array}  The ordinal text (e.g. st, nd, rd, th) for digits 1 to 9
		 */
		ORDINALNAMES    : 'ro ero do ero to to to mo vo no'.split(' '),
		/**
	...
	Date.create.patterns = [
		...
		['chicago', /^(?:(?:lun|mar|mie|jue|vie|sab|dom)[a-z]*,?\s+)?(3[01]|[0-2]\d|\d)\s*([ -])\s*(?:(ene|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic)[a-z]*)\s*\2\s*([1-9]\d{3})$/i, '$3 $1, $4'],

		// "March 4, 2012", "Mar 4 2012", "Sun Mar 4 2012"
		['conversational', /^(?:(?:lun|mar|mie|jue|vie|sab|dom)[a-z]*,?\s+)?(?:(ene|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic)[a-z]*)\s+(3[01]|[0-2]\d|\d),?\s*([1-9]\d{3})$/i, '$1 $2, $3'],
		...
		['this_next_last',
		/^(this|next|last)\s+(?:(year|month|week|day|hour|minute|second|millisecond)|(ene|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic)|(dom|lun|mar|mie|jue|vie|sab))/i,
		function(match) {
			// $1 = this/next/last
		...
		/^(?:(ene|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic)[a-z]*\s+)(?:the\s+)?(\d+)(?:st|nd|rd|th)?$/i,
		function(match) {
			var d = Date.current();
			if (match[1]) {
